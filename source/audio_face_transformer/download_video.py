...
def find_mp4_and_copy_to_folder(
    copy_file_name=None, move_to_folder=config.INPUT_FOLDER_PATH
):
    for file in os.listdir():
        if file.endswith(".mp4"):
            ...
            shutil.copyfile((file), move_to_folder + copy_file_name)
            try:
                os.remove(file)
            except:
                return None
                pass
            print(
                f"File has been moved to the folder {move_to_folder.split(os.sep)[-2:]}. File name: ",
                file,
            )
            return file_name
...
def get_links_from_csv(csv_file, link_column, move_to_folder):
    ...
    for i, link in enumerate(links):
        try:
            ...
            find_mp4_and_copy_to_folder(
                copy_file_name=copy_file_name, move_to_folder=move_to_folder
            )
            cnt += 1
        except:
            traceback.print_exc()
...
if __name__ == "__main__":
    get_links_from_csv(
        config.INPUT_FOLDER_PATH + "mosei.csv",
        1,
        move_to_folder=config.INPUT_FOLDER_PATH + "mosei" + os.sep,
    )
