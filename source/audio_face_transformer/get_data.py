...
import source.audio_face_transformer.utils as utils
    ...
    for i, folder in enumerate(savee_paths):
        ...
        for j, file in enumerate(sub_folder_path):
            ...
            utils.convert_video_to_mp4(
                video_path=folder_path + config.ls + folder + config.ls + file,
...
def convert_OMG(folder_path, save_path, labels_csv, max_threads=10):
    ...
    def split_video_into_custom_parts(
        divide_array, save_names, full_video, save_dir, delete_folder_contents_flag=True
    ):
    ...
    def get_file_time_emotion_pair(rows, max_time=tr_config.VIDEO_ANALYSE_WINDOW_SECS):
        ...
        for i, row in enumerate(rows):
            ...
            if end_time - start_time < max_time:
                ...
                while True:
                    ...
                    file_name_save = (
                        f"OMG_{cnt_rows}-{start_time_loop}-{end_time_loop}_{emotion}"
                    )
                    file_and_save_names_pairs[file_name].append(file_name_save)

                    # If the end time is greater than the max time, then set the end time to max time
                    file_and_times_pairs[file_name].append(
    ...
    for i, file_name_target in enumerate(file_and_save_names_pairs):
        ...
        for j, file_name_save in enumerate(file_and_save_names_pairs[file_name_target]):
            ...
            print(
                f"Start time: {file_and_times_pairs[file_name_target][j][0]}, End time: {file_and_times_pairs[file_name_target][j][1]}"
            )
            print()
        ...
        threading.Thread(
            target=split_video_into_custom_parts,
            args=(
                (
                    file_and_times_pairs[file_name_target],
...
def convert_CREMAD(folder_path, save_path):
    ...
    for i, file in enumerate(folder_files_list):
        print("Converting " + file)
        shutil.copyfile((folder_path + file), save_path + get_label_schema(file, cnt))
        cnt += 1
    print("Conversion complete")


def split_to_test_and_train(
    data_path, test_path, train_path, test_per=config.test_split_percentage
):
    ...
    for i, file in enumerate(data_paths):
        ...
        if i < len(data_paths) * test_per:
            ...
            shutil.copyfile((data_path + file), test_path + file)
        else:
            print(f"Copying {file} to train")
            shutil.copyfile((data_path + file), train_path + file)
...
if __name__ == "__main__":
    ...
    convert_OMG(
        folder_path=config.OMG_ORIGINAL_VIDEOS_FOLDER_PATH,
        save_path=config.OMG_EXTRACTED_AV_FOLDER_PATH + "train" + os.sep,
        labels_csv=config.OMG_labels_file_paths,
    )
