...
def __convert_video_to_frames__(
    cv2_capture,
    frame_rate,
    crop_video_length,
    ...
    frame_rate_crop_video_length = round(
        frame_rate * crop_video_length
    )  # Target length of each clip expressed in frames
    ...
    if return_consistent_length:
        ...
        for i in range(len(frames_clips[0]), frame_rate_crop_video_length):
            frames_clips[0].append(
                np.zeros(frames_clips[0][0].shape, dtype=type(frames_clips[0][0][0][0]))
            )
    ...
    if print_flag:
        ...
        print(
            "Small clip has length: ",
            min(len_of_clips),
            " at index: ",
            len_of_clips.index(min(len_of_clips)),
        )
        print(
            "Big clip has length: ",
            max(len_of_clips),
            " at index: ",
            len_of_clips.index(max(len_of_clips)),
        )
...
def __convert_audio_to_clips__(
    signal,
    sample_rate,
    crop_audio_length,
    return_consistent_length=True,
    print_flag=True,
):
    """
    ...
    """
    sample_rate_crop_audio_length = round(
        sample_rate * crop_audio_length
    )  # Target length of each clip expressed in samples
    ...
    if return_consistent_length:
        # make the smallest clip the same length
        signal_clips[0] = audio_utils.make_signal_len_consistent(
            signal_clips[0], sample_rate_crop_audio_length
        )
...
def convert_video_to_frames_and_audio(
    ...
    min_vid_len=config.MIN_VID_LEN,
    print_flag=True,
    recursion_depth=1,
):
    """
    ...
    try:
        ...
        video_length = int(cap.get(cv2.CAP_PROP_FRAME_COUNT)) / int(
        ...
        arrays_of_frames = __convert_video_to_frames__(
            cap, frame_rate, crop_video_length, print_flag=print_flag
        )
        ...
        audio, sample_rate = librosa.load(tempfile, sr=sample_rate)
        try:
            os.remove(tempfile)
        except:
            pass
        arrays_of_audio_clips = __convert_audio_to_clips__(
            audio, sample_rate, crop_video_length, print_flag=print_flag
        )
        ...
        return convert_video_to_frames_and_audio(
            video_path,
            crop_video_length,
            frame_rate,
...
def split_video_into_equal_parts(
    full_video="full.mp4",
    save_dir="",
    divide_into_secs=transformer_config.VIDEO_ANALYSE_WINDOW_SECS,
    min_vid_len=config.MIN_VID_LEN,
):
...
def split_video_into_custom_parts(
    ...
    full_video="full.mp4",
    save_dir="",
    delete_folder_contents_flag=False,
):
    """
    Parameters:
...
def get_input_shape(which_input):
    if which_input == "landmarks_depths":
        ...
        return transformer_config.AUDIO_INPUT_SHAPE
    elif which_input == "text_sentiment":
        return transformer_config.TEXT_SENTIMENT_INPUT_SHAPE
...
def visualise_emotions(video_path, csv_path):
    ...
    with open(csv_path, "r") as file:
        ...
        for i, row in enumerate(reader):
            ...
            emotions_values = {
                emotions[i]: float(val.split(":")[1].replace("%", ""))
            ...
            # Wait for time difference between videos if some seconds are skipped
                ...
                print(
                    "Waiting for {} seconds. Skipped seconds".format(
                ...
                update_bar_chart(
                    empty_emotions,
                    title=f"Skipped {last_end_seconds} to {start_seconds}",
                )
            ...
            print(
                "Waiting for {} seconds. Playing for {} to {}".format(
